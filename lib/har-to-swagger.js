...
HarToSwagger.addParameterFromBody = function(swaggerMethod, swagger, harLogEntry, options) {
  if (harLogEntry.request.bodySize > 0) {
    ...
    if (bodyType) {
      this.addDefinition(requestBodyJson, bodyType.value, swagger, harLogEntry, options);
      requestBodySchema = {
...
HarToSwagger.addResponse = function (swaggerMethod, swagger, harLogEntry, options) {
  if (harLogEntry.response.bodySize > 0) {
    ...
    if (bodyType) {
      this.addDefinition(responseBodyJson, bodyType.value, swagger, harLogEntry, options);
      responseBodySchema = {
        "$ref": "#/definitions/" + bodyType.value
      }
    }

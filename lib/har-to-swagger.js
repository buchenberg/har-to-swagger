...
HarToSwagger.addResponse = function (swaggerOperation, swagger, harLogEntry, options) {
    ...
    if (harLogEntry.response.bodySize > 0 && harLogEntry.response.content.text !== undefined) {
        ...
        var reHTML  = new RegExp("/text\/html.*/");
        if ((m = reHTML.exec(str)) !== null) {
            if (m.index === reHTML.lastIndex) {
                re.lastIndex++;
            }
        ...
        } else if (reHTML.test(JSON.stringify(harLogEntry.response.content.mimeType))) {

...
HarToSwagger.addOperation = function (swagger, harLogEntry, options) {
  ...
  var operation = swagger.paths[url.pathname][harLogEntry.request.method.toLowerCase()];
...
HarToSwagger.addResponse = function (swaggerOperation, swagger, harLogEntry, options) {
  if (harLogEntry.response.bodySize > 0) {
    ...
    // TODO: right now 1 operation only gets 1 response. should support multiple responses.
